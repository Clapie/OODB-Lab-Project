...
namespace Discover_O_laptop.Forms
{
    public partial class ViewHistoryForm : Form
    {
        ...
        public void loadData()
        {
            if (month != 0)
            {
                var obj = (from x in de.HeaderTransactions join y in de.Users on x.UserID equals y.UserID
            ...
            if (transactionID != "")
            {
                var obj = (from x in de.DetailTransactions where x.TransactionID.Equals(transactionID)
                           select new
                           {
                               LaptopID = x.LaptopID,
                               Quantity = x.Quantity
